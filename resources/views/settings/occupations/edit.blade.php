<x-app-layout>
    <div class="py-6">
        ...
        <div class="flex items-center pb-6 justify-between">
            ...
            <x-link-button.secondary-link :href="route('settings.classrooms.index')">
                Back
        ...
        <div class="">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <form action="{{ route('settings.classrooms.update', $classroom->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="input-group">
                                ...
                                <x-form.select-input id="level" name="level" class="mt-1">
                                    @foreach (classroomLevels() as $level)
                                        <option value="{{ $level }}" @selected(old('level', $classroom->level) == $level)>
                                            {{ $level }}
                                        </option>
                                    @endforeach
                                </x-form.select-input>
